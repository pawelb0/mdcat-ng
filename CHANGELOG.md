...
- Interactive `mdless` viewer (vi-style scrolling, smart-case / regex
  search with SGR-aware highlight, heading jumps, TOC modal, bookmarks).
- Sixel graphics protocol via `icy_sixel`.
- tmux and GNU screen passthrough (`$TMUX` / `$STY`).
- DA1 capability probing on Unix (falls back to plain ANSI off).
- Detection for Alacritty, Apple Terminal, Contour, Foot, Hyper, Konsole,
  mlterm, Rio, Warp, Windows Terminal.
- `--remote-images` flag; remote fetches off by default.
- `--external-pager`, `--no-pager` (`-P`), `--probe-timeout-ms`,
  `--no-probe-terminal`, `--wrap-code`, `--line-numbers` (`-n`).
- GFM alert blockquotes, footnotes, definition lists, wiki links,
  smart punctuation.
- Public `mdcat::mdless`, `mdcat::RenderObserver`, `Multiplexer`.

### Changed

- `pulldown-cmark-mdcat` folded into `mdcat`.
- `push_tty` returns `RenderResult<()>` instead of `std::io::Result<()>`.
- Headings: H1 `═══`, H2 `───`, H3–H6 bold colour.
- Blockquotes get a `▌ ` accent bar. Tables and code blocks use Unicode
  box drawing.
- Pipe-safe output by default — styling and image escapes drop on
  non-TTY stdout.
- Dependency majors bumped (rustix, pulldown-cmark, resvg, gethostname,
  similar-asserts, terminal_size).

### Removed

- `Osc8Links` marker struct (unused).
- Explicit `glob` dev-dependency.
...
- Buffer writes to terminal and `less`, to reduce the amount of syscalls.
...
- Add `--paginate` flag to paginate the output of mdcat (see [#152]);
  with this flag mdcat sends its output to `less -R` or any alternative set in `$MDCAT_PAGER` or `$PAGER`.
  This flag also disables all terminal-specific formatting since downstream tools likely won't support it.
